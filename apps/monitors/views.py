...
from apps.common.web import AdminRequiredMixin, paginate_collection
...
class MonitorAdminView(AdminRequiredMixin, TemplateView):
    ...
    def _base_queryset(self):
        queryset = Monitor.objects.select_related("user").order_by("full_name")
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(
            {
                "create_form": kwargs.get("create_form") or MonitorRegistrationForm(),
                ...
                "stats": {
                    ...
                    "active": Monitor.objects.filter(is_active=True, user__is_active=True).exclude(user__password__startswith="!").count(),
                    "pending": Monitor.objects.filter(is_active=True, user__is_active=True, user__password__startswith="!").count(),
                    "inactive": Monitor.objects.filter(Q(is_active=False) | Q(user__is_active=False)).count(),
                },
                **pagination,
            }
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "create":
            form = MonitorRegistrationForm(request.POST)
            if form.is_valid():
                create_monitor_with_user(**form.cleaned_data, request=request)
        ...
        if action == "upload":
            ...
            if form.is_valid():
                try:
                    result = import_monitors_from_workbook(uploaded_file=form.cleaned_data["source_file"], request=request)
        ...
        monitor = get_object_or_404(Monitor.objects.select_related("user"), pk=request.POST.get("monitor_id"))
        if action == "toggle":
